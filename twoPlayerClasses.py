...
class Block:
    ...
    def move(self, tiles, player):
        ...
        if self.player == 1:
            ...
            if keys[pygame.K_s] and not self.top_collisions(tiles, player) and not self.stationary:
                self.speed = 5
            else:
                self.speed = 25
            if keys[pygame.K_d] and not self.right_collisions(tiles, player) and not self.stationary:
                if self.aTick % 5 == 0:
                    for block in self.blocks:
                        block.x += tileSize
                    self.x += tileSize
                    self.aTick = 0
            if keys[pygame.K_a] and not self.left_collisions(tiles, player) and not self.stationary:
                if self.aTick % 5 == 0:
                    for block in self.blocks:
                        block.x -= tileSize
                    self.x -= tileSize
    ...
    def test_collisions(self, player):
        for block in self.blocks:
            if self.player == 2:
                if block.x == 400 or block.x == 130 or block.y == 670:
                    return True
                else:
                ...
                if block.x == 640 or block.x == 370 or block.y == 670:
                    return True
                else:
                    for pBlock in player.blocks:
                        for tile in pBlock.blocks:
    ...
    def left_collisions(self, tiles, player):
        if self.player == 1:
            for block in self.blocks:
                for tile in tiles[self.row]:
                    if block.x <= 400 and tile.type == 'wall':
                        return True
                for pBlock in player.blocks:
                    for tile in pBlock.blocks:
